...
export class ConsoleDataProvider {
  ...
  private async handleConsoleMessage(message: any): Promise<void> {
    ...
    const _consoleError = DataAdapters.consoleErrorToV1(message.data);

    // Update cache
    const _cacheKey = `tab_${ consoleError.tabId || 'global' }`;
    if (!this.cache.has(cacheKey)) {
      this.cache.set(cacheKey, []);
    }

    const _tabErrors = this.cache.get(cacheKey)!;
  ...
  async getConsoleErrors(options: {
    ...
    try {
      ...
      let _errors = this.cache.get(cacheKey) || [];
  ...
  private applyFilters(
    ...
    return errors.filter(error => {
      ...
      if (filters.timeRange) {
        const _errorTime = new Date(error.timestamp);
  ...
  async storeConsoleError(error: Partial<ConsoleErrorV1>): Promise<boolean> {
    try {
      const _response = await ConsoleMessageBus.sendConsoleError(error);
  ...
  getConsoleStats(tabId?: number): {
    ...
    const _topSources = Object.entries(sourceCounts)
    ...
    const _recentErrors = errors.filter(e => new Date(e.timestamp) > oneHourAgo);
    const _recentErrorRate = recentErrors.length; // errors per hour

    // Critical error count (errors with stack traces)
    const _criticalErrorCount = errors.filter(e => e.level === 'error' && e.stack).length;
...
export const _consoleDataProvider = ConsoleDataProvider.getInstance();
