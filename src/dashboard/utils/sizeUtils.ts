...
export function getSizeBreakdown(req: NetworkRequestWithSize): {
  ...
  const _payloadSize = parseSize(req.payload_size);
  const _requestSize = parseSize(req.requestSize || req.request_size);
  const _responseSize = parseSize(req.responseSize || req.response_size);
  const _totalCalculated = requestSize + responseSize;

  // Estimate from body content
  let _estimatedFromBody = 0;
  const _requestBody = req.requestBody || req.request_body;
  const _responseBody = req.responseBody || req.response_body;
  ...
  const _standardizedSize = getStandardizedSize(req);
