  ...
  if (selectedField === 'body') {
    ...
    const isStatusOnlyResponse = (body: any): boolean => {
      ...
      try {
        ...
        // Not valid JSON, continue checking for status patterns
      }
      ...
      if (hasActualJsonStructure) {
        return false; // It's likely JSON content, not a status message
      }
      ...
      const statusOnlyPatterns = [
        ...
        /^[\s]*\d{1,3}[\s]*$/,                 // "200", " 404 ", "0", "204", etc.
        /^[\s]*\d{1,3}\s+[A-Za-z\s]+[\s]*$/,  // "404 Not Found", "200 OK", "0 Network Error", etc.
        /^[\s]*[A-Za-z\s]+[\s]*$/,            // "OK", "Not Found", "Unauthorized", etc.
        /^[\s]*\d{1,3}:\s*[A-Za-z\s]+[\s]*$/, // "404: Not Found", "0: Network Error", etc.
        /^[\s]*Status:\s*\d{1,3}\s+[A-Za-z\s]+[\s]*$/,  // "Status: 404 Not Found", "Status: 0 Network Error"
        /^[\s]*Status:\s*\d{1,3}[\s]*$/,      // "Status: 404", "Status: 0", "Status: 204"
      ];
    ...
    const getBodyExplanation = (body: any, status?: number): string => {
      if (isStatusOnlyResponse(body)) {
        if (status) {
          ...
          if (status >= 300 && status < 400) {
            ...
            if (status === 302) return "↪️ Found (Temporary Redirect) - Response body contains redirect status only";
            if (status === 304) return "↪️ Not Modified - No response body, resource unchanged";
            if (status === 307) return "↪️ Temporary Redirect - Response body contains redirect confirmation";
            if (status === 308) return "↪️ Permanent Redirect - Response body contains redirect status";
            return "↪️ Redirect response - Response body contains minimal redirect information";
          }
          
          // 4xx Client Error responses
          if (status >= 400 && status < 500) {
            if (status === 400) return "❌ Bad Request - Response body may contain error details in non-JSON format";
            if (status === 401) return "🔐 Unauthorized - Response body contains authentication error (possibly encrypted)";
            if (status === 403) return "� Forbidden - Response body contains access denial information";
            if (status === 404) return "🔍 Not Found - Response body contains minimal error information";
            if (status === 405) return "❌ Method Not Allowed - Response body contains method restriction details";
            if (status === 408) return "⏱️ Request Timeout - Response body contains timeout information";
            if (status === 409) return "⚡ Conflict - Response body contains conflict resolution details";
            if (status === 410) return "�️ Gone - Response body contains resource removal confirmation";
            if (status === 413) return "📏 Payload Too Large - Response body contains size limit information";
            if (status === 415) return "📋 Unsupported Media Type - Response body contains format error details";
            if (status === 422) return "📝 Unprocessable Entity - Response body contains validation errors";
            if (status === 429) return "🚦 Too Many Requests - Response body contains rate limit information";
            return "❌ Client Error - Response body may contain error details in protected format";
          }
          
          // 5xx Server Error responses
          if (status >= 500) {
            if (status === 500) return "💥 Internal Server Error - Response body contains server error details";
            if (status === 501) return "🚧 Not Implemented - Response body contains capability information";
            if (status === 502) return "🔗 Bad Gateway - Response body contains proxy/gateway error details";
            if (status === 503) return "⏸️ Service Unavailable - Response body contains service status information";
            if (status === 504) return "⏱️ Gateway Timeout - Response body contains timeout error details";
            if (status === 507) return "💾 Insufficient Storage - Response body contains storage error information";
            return "💥 Server Error - Response body contains server error information";
          }
        }
        
        // Fallback explanations when no status code is available
        const explanations = [
      ...
      return "Content not available for display";
    };
              ...
              <div className="flex items-center space-x-2">
                ...
                {isStatusOnlyResponse(responseBody) && (
                  <div className="relative group">
                    ...
                    <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 w-64 bg-gray-800 text-white text-xs rounded-lg p-3 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                ...
                <div className="flex items-start space-x-3">
                  <div className="flex-1">
                    ...
                    <div className="mt-2 text-xs text-yellow-600">
                      <p>The server returned a simple status message instead of structured data. This is common for:</p>
                      <ul className="list-disc list-inside mt-1 space-y-1">
                        <li>Authentication endpoints (login/logout)</li>
                        <li>File upload confirmations</li>
                        <li>Error responses with minimal content</li>
...
const Dashboard: React.FC = () => {
  ...
  // MEMORY LEAK FIX: On-demand page loading functions
  const loadNetworkRequestsPage = useCallback(async (page: number, limit: number = 10) => {
    try {
      ...
      if (response?.success && response?.requests) {
        // Update only the data for the current page, discard previous page data
        setData(prevData => ({
          ...prevData,
  ...
  const loadDashboardData = useCallback(async () => {
    try {
      ...
      // MEMORY LEAK FIX: Only get counts, don't load page data to avoid overwriting current page
      // Get total counts without loading actual data
      const countsResponse = await sendChromeMessage({ 
        action: 'getTableCounts'
      })
      
      console.log('📊 Dashboard counts response:', countsResponse)
      
      // MEMORY LEAK FIX: Update only metadata and totals, preserve current page data
      setData(prevData => ({
        ...prevData, // Preserve current page data (networkRequests, consoleErrors, tokenEvents)
        ...
        lastActivity: storageData.lastActivity 
          ...
          : 'Never',
        // Update only the totals from counts response
        totalRequests: countsResponse?.data?.apiCalls || 0,
        totalErrors: countsResponse?.data?.consoleErrors || 0,
        totalTokenEvents: countsResponse?.data?.tokenEvents || 0
      }))
      
      console.log('✅ Dashboard: Metadata loaded successfully', {
        totalTabs: tabs.length,
        totalRequests: countsResponse?.data?.apiCalls || 0,
        totalErrors: countsResponse?.data?.consoleErrors || 0,
        totalTokenEvents: countsResponse?.data?.tokenEvents || 0
  ...
  }, [currentTokenPage, tokenEventsPerPage, loadTokenEventsPage, data.totalTokenEvents]);
  ...
  // Use database total for pagination, not filtered client-side count
  const totalFilteredRequests = data.totalRequests || 0; // Server-side pagination total
  ...
  // Use database total for pagination, not filtered client-side count
  const totalFilteredErrors = data.totalErrors || 0; // Server-side pagination total
  ...
  // For display purposes, use the current page data directly (no client-side slicing needed)
  const currentErrors = filteredAndSortedErrors; // Data is already paginated from server
  ...
  }, [errorSearchTerm, filterSeverity]);
  ...
  }, [tokenSearchTerm, filterTokenType]);
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-5">
          ...
          <div className="bg-white rounded-lg shadow p-6 cursor-pointer hover:bg-gray-50 transition-colors" onClick={() => window.open('about:blank', '_blank')}>
            <div className="flex items-center">
              <div className="flex-shrink-0">
                <div className="w-8 h-8 bg-blue-500 rounded-lg flex items-center justify-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-semibold text-gray-900">View Details</p>
              </div>
            </div>
          </div>

          <div className="bg-white rounded-lg shadow p-6 cursor-pointer hover:bg-gray-50 transition-colors" onClick={() => window.open('/src/settings/settings.html', '_blank')}>
            <div className="flex items-center">
              <div className="flex-shrink-0">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-semibold text-gray-900">Configure</p>
              </div>
            </div>
          </div>
        </div>
                    ...
                    >
                      <span className="flex items-center space-x-2">
                        ...
                        <span className={`inline-flex items-center justify-center px-2 py-1 text-xs font-medium rounded-full ${
                          ...
                          {index === 0 ? (data.totalRequests || 0) : 
                           index === 1 ? (data.totalErrors || 0) : 
                           index === 2 ? (data.totalTokenEvents || 0) :
                           'LIVE'}
                        </span>
                      </span>
                    </button>
