...
// CRITICAL FIX: Add missing console error handler for main world script
eventHandlers.consoleIntercepted = async (event: any) => {
  ...
  try {
    ...
    // Format data for CONSOLE_ERROR handler compatible with our IndexedDB storage
    const formattedData = {
    ...
    console.log('🔥 CONTENT: Received console error from main-world:', formattedData);

    // Send to background script using CONSOLE_ERROR action
    ...
    console.warn('❌ CONTENT: Error handling console error:', error);
    extensionContextValid = false;
  }
};

// Add the console error event listener
